...
import { HomeLifecycleInteractive } from '@/components/home-lifecycle-interactive';
...
export default function HomePage() {
  return (
    <main id="main">
        ...
        <div className="relative mx-auto max-w-[1440px] px-5 py-24 md:px-8 md:py-32 lg:px-12 lg:py-40">
          <div className="grid gap-14 xl:grid-cols-[minmax(0,0.92fr)_minmax(460px,0.88fr)] xl:items-end">
            <div className="max-w-5xl">
              <div className="mb-6 text-sm uppercase tracking-[0.22em] text-textSecondary">
              ...
              <div className="mt-10 flex flex-wrap gap-4">
                <a
                  href="#system"
                  className="rounded-full border border-accentGreen/40 bg-accentGreen/10 px-6 py-3 text-sm font-medium text-white transition hover:bg-accentGreen/20"
                >
                  Explore the system
                </a>
                <a
                  href="#interactive-lifecycle"
                  className="rounded-full border border-white/15 px-6 py-3 text-sm font-medium text-textPrimary transition hover:border-white/30 hover:bg-white/5"
                >
      ...
      <SectionShell
        eyebrow="The break"
        title="Why marketing feels busy but does not compound"
        intro="Campaigns run, but do not connect. Messaging exists, but does not differentiate. Data is collected, but not used to decide."
      >
        <div className="space-y-5">
          <p className="max-w-content text-lg leading-8 text-textSecondary">
            Marketing becomes a series of isolated actions instead of a system.
          </p>
          <div className="rounded-[28px] border border-white/10 bg-surface p-6 shadow-soft">
      ...
      <SectionShell
        eyebrow="The shift"
        title="Marketing starts working when it becomes a system"
        intro="Not a set of activities. A structure."
      >
        <div className="space-y-6">
          <p className="max-w-content text-lg leading-8 text-textSecondary">
            A system connects what you learn from the market, how you define value, how decisions are made,
            how actions are triggered, how results are measured, and how all of it improves over time.
          </p>
          <div className="grid gap-4 md:grid-cols-5">
            {['Signals', 'Decisions', 'Actions', 'Outcomes', 'Learning'].map((item) => (
              <div
                key={item}
                className="rounded-[22px] border border-white/10 bg-white/[0.03] px-4 py-5 text-center text-sm uppercase tracking-[0.18em] text-textPrimary"
              >
                {item}
              </div>
            ))}
      ...
      >
        <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          <Callout title="What the lifecycle guide covers">
            ...
            >
              Explore the interactive lifecycle
            </a>
          </Callout>
      ...
      <SectionShell
        eyebrow="The real insight"
        title="Marketing does not end at purchase."
        intro="Growth is not created only before the sale."
      >
        <div className="space-y-5">
          <ul className="space-y-3 text-lg leading-8 text-textSecondary">
